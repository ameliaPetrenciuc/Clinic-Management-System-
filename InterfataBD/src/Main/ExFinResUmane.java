...
public class ExFinResUmane extends javax.swing.JFrame {
    ...
    public ExFinResUmane(int id_utilizator, int functie) {
    ...
    private void jLabel8MouseClicked(MouseEvent evt) {
        Connection connection = ConexiuneSQL.Conexiune();
        afisareConcediu(connection);
    }
    private void afisareConcediu(Connection connection) {
        try {
            String query = "CALL afiseaza_concedii_utilizator("+id_utilizator+");";///////////////aici in aprenteaza va fi id ul tilizatorului ce foloseste aplicatia
            try (PreparedStatement pst = connection.prepareStatement(query);
                ...
                JTable userTable = new JTable(buildTableModelConcedii(rs));
    ...
    public static DefaultTableModel buildTableModelConcedii(ResultSet rs) throws SQLException {
        ResultSetMetaData metaData = rs.getMetaData();
...
//vezi orar
    private void jLabel4MouseClicked(MouseEvent evt) {
        ExFinVeziOrar op  = new   ExFinVeziOrar(id_utilizator, functie);
        op.setVisible(true);
        dispose();
    }
  ...
  /*  private void afisareVeziOrar(Connection connection) {
        try {
            String query = "{CALL vezi_orar(7,'2023-01-03')}";///////////////aici in aprenteaza va fi id ul tilizatorului ce foloseste aplicatia
            try (PreparedStatement pst = connection.prepareStatement(query);
                 ResultSet rs = pst.executeQuery()) {

                // Create a new frame to display the user table
                JFrame userTableFrame = new JFrame("Afisare Orar");
                userTableFrame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
                userTableFrame.setSize(1200, 500);
...
*/
    ...
    /*public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(() -> {
            new ExFinResUmane().setVisible(true);
        });
    }*/
