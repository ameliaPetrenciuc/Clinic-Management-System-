...
public class AddInspector extends javax.swing.JFrame {

    public int admin_lvl;
    Connection connection = null;
    PreparedStatement prp = null;

    public AddInspector(int admin_lvl) {
    ...
    private void initComponents() {
        ...
        jLabel13.setText("Adaugare Inspector");
        ...
        addReceptionerButton.setText("Adaugare Inspector");
        addReceptionerButton.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                addInspectorbtnMouseClicked(evt);
            }
        });
    ...
    private void jLabel21MouseClicked(MouseEvent evt) {
        OptiuniAdminInspector opp = new OptiuniAdminInspector(admin_lvl);
        opp.setVisible(true);
        dispose();
    }


    private void addInspectorbtnMouseClicked(MouseEvent evt) {
        try {
            ...
            try (Connection connection = ConexiuneSQL.Conexiune()) {
                if (connection != null) {
                    ...
                    try (PreparedStatement pst = connection.prepareStatement(sql)) {
                        ...
                        pst.execute();
                        JOptionPane.showMessageDialog(null, "Inspector adăugat cu succes!");
                        afisareInspector(connection);
                    }
                }
    ...
    private void afisareInspector(Connection connection) {
        try {
            String query = "{CALL AFISARE_INSPECTOR()}";
            try (PreparedStatement pst = connection.prepareStatement(query);
                 ResultSet rs = pst.executeQuery()) {

                // Create a new frame to display the user table
                JFrame userTableFrame = new JFrame("Afisare Inspectori");
    ...
    /*public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            new AddInspector().setVisible(true);
        });
    }*/
